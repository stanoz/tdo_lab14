...
beforeAll(async () => {
    await db.query('CREATE TABLE IF NOT EXISTS texts (id SERIAL PRIMARY KEY, content TEXT NOT NULL)');
});
...
describe('POST /texts and GET /texts', () => {
    it('should insert and retrieve texts', async () => {
        const postRes = await request(app).post('/texts').send({ text: 'Hello, world!' });
